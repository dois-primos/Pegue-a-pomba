export default class fase5 extends Phaser.Scene {
  ...
  create () {
    ...
    this.spawnPassaro = () => {
      ...
      const tipoPassaro = Phaser.Math.Between(0, 2) === 0 ? 'pomba-branca' :
        Phaser.Math.Between(0, 1) === 0 ? 'pomba-cinza' : 'corvo';
      const animacao = tipoPassaro === 'pomba-branca' ? 'voar-branca' :
        tipoPassaro === 'pomba-cinza' ? 'voar-cinza' : 'voar-corvo';
  ...
  update (time, delta) {
    ...
    if (!this.aguardandoNovaRodada && this.passarosRestantes > 0) {
      ...
      if (this.tempoParaNovoPassaro > 1200) { // Menor tempo entre os pássaros
        this.tempoParaNovoPassaro = 0;
